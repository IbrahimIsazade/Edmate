...
{
    class RefreshTokenRequestHandler(ICryptoService cryptoService, UserManager<CustomUser> userManager, DbContext db, IHttpContextAccessor ctx)
        : IRequestHandler<RefreshTokenRequest, AuthenticateResponse>
    {
        public async Task<AuthenticateResponse> Handle(RefreshTokenRequest request, CancellationToken cancellationToken)
        {
            ...
            var accessTokenEntry = await db.Set<CustomUserToken>().FirstOrDefaultAsync(m => m.UserId == numUserId &&
            ...
            var refreshTokenEntry = await db.Set<CustomUserToken>().FirstOrDefaultAsync(m => m.UserId == numUserId &&
            ...
            await db.Set<CustomUserToken>().AddRangeAsync([
                new CustomUserToken{ UserId =user.Id, LoginProvider="APPLICATION",Name="ACCESS_TOKEN",Value =cryptoService.Sha1Hash(response.AccessToken),Type=TokenType.AccessToken,ExpireDate=accessTokenExpireDate,IsDisable=false  },
                new CustomUserToken{ UserId =user.Id, LoginProvider="APPLICATION",Name="REFRESH_TOKEN",Value =cryptoService.Sha1Hash(response.RefreshToken),Type=TokenType.RefreshToken,ExpireDate=accessTokenExpireDate.AddHours(2),IsDisable=false  },
                ], cancellationToken);
